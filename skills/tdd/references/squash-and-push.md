...
- Preserve **story** during development (`test:` → `feat:` → `refactor:`).
- Publish **one** cohesive commit per completed cycle, titled like the **GREEN** `feat:` line.
...
- **Primary title** mirrors the GREEN `feat:` subject (polish allowed, meaning must match).
- Do **not** lead with `test:` or `refactor:` in the squashed title; the squashed commit represents **delivered behavior**.
