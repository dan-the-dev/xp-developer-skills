...
3. **Verify every change** — If it isn’t testable, reviewable, and incremental, it isn’t done. Re-run affected tests after every edit; choose the **right test layers and techniques** per slice ([`test-strategy-selection.md`](test-strategy-selection.md)); run **all verify steps the current project defines** at slice boundary ([`delivery-process.md`](delivery-process.md), [`project-verification.md`](project-verification.md)). See [`technical-excellence-catalog.md`](technical-excellence-catalog.md) for the practice inventory we amplify.
